CREATE OR REPLACE PACKAGE BODY stpks_stdgris2_main AS
   ...
   BEGIN
      ...
      LOOP
         ...
         ELSIF  l_Node = 'BLK_DETAIL' THEN
            ...
            p_stdgris2.v_sttms_grisela_detail(l_Dsn_Rec_Cnt_2).CATEGORY := Cspks_Req_Global.Fn_GetVal;
            p_stdgris2.v_sttms_grisela_detail(l_Dsn_Rec_Cnt_2).PRODUCT_CODE := Cspks_Req_Global.Fn_GetVal;
            p_stdgris2.v_sttms_grisela_detail(l_Dsn_Rec_Cnt_2).category :=p_stdgris2.v_sttms_grisela_ushtrimi2.category;
         END IF;
         l_Node := Cspks_Req_Global.Fn_GetNode;
      END LOOP;
      ...
      LOOP
         ...
         ELSIF  l_Node IN ( 'BLK_DETAIL','Detail') THEN
            ...
            LOOP
               --dbg('Key/Value   :'||l_Key ||':'||l_Val);
               IF l_Key = 'CATEGORY' THEN
                  p_stdgris2.v_sttms_grisela_detail(l_Dsn_Rec_Cnt_2).CATEGORY := l_Val;
   ...
   BEGIN
      ...
      IF p_stdgris2.v_sttms_grisela_detail.COUNT > 0 THEN
         FOR i_2 IN  1..p_stdgris2.v_sttms_grisela_detail.COUNT LOOP
            ...
            Cspks_Req_Global.Pr_Write('V','CATEGORY',p_stdgris2.v_sttms_grisela_detail(l_Dsn_Rec_Cnt_2).category);
   ...
   BEGIN
      ...
      IF SUBSTR(p_Exchange_Pattern,3,4) = 'FS' THEN
         ...
         --Dbg('Building Childs Of :..');
         IF (  p_stdgris2.v_sttms_grisela_ushtrimi2.category IS NOT NULL 
          )
          THEN
            l_1_Lvl_Counter := 0;
            l_0_Lvl_Counter   := l_0_Lvl_Counter +1;
      ...
      ELSE
         ...
         l_Key_Cols := 'CATEGORY~';
         l_Key_Vals := p_stdgris2.v_sttms_grisela_ushtrimi2.category||'~';
         ...
         FOR l_index  IN 1 .. l_count LOOP
            ...
            IF l_index < l_Count THEN
               FOR l_index1 IN l_index+1 .. l_Count LOOP
                  IF (NVL(p_stdgris2.v_sttms_grisela_detail(l_index).category,'@')=  NVL(p_stdgris2.v_sttms_grisela_detail(l_index1).category,'@')) AND (NVL(p_stdgris2.v_sttms_grisela_detail(l_index).product_code,'@')=  NVL(p_stdgris2.v_sttms_grisela_detail(l_index1).product_code,'@')) THEN
                     Dbg('Duplicare Record Found for :'||l_Key);
                     l_key := Cspks_Req_Utils.Fn_Get_Item_Desc(p_source,g_ui_name,'STTMS_GRISELA_DETAIL.CATEGORY')||'-'||
                     p_stdgris2.v_sttms_grisela_detail(l_index).category||':'||
   ...
   BEGIN
      ...
      IF p_Source <> 'FLEXCUBE'  THEN
         ...
         END;
      END IF;
      l_key := Cspks_Req_Utils.Fn_Get_Item_Desc(p_source,g_ui_name,'STTMS_GRISELA_USHTRIMI2.CATEGORY')||'-'||
      p_stdgris2.v_sttms_grisela_ushtrimi2.category;
      l_Prev_Key_Tags := 'CATEGORY~';
      l_Prev_Key_Vals := p_prev_stdgris2.v_sttms_grisela_ushtrimi2.category||'~';
   ...
   RETURN BOOLEAN IS
      ...
      WHERE category = p_wrk_stdgris2.v_sttms_grisela_ushtrimi2.category
      ;
   BEGIN
      Dbg('In Fn_Sys_Query..');
      ...
      ELSE
         ...
         IF NVL(p_With_Lock,'N') = 'Y' THEN
            BEGIN
               ...
               WHERE category = p_stdgris2.v_sttms_grisela_ushtrimi2.category
                FOR UPDATE NOWAIT;
            EXCEPTION
...
;
         ...
         IF l_Del_Count > 0 THEN
            FOR l_index IN 1 .. l_del_count LOOP
               ...
               WHERE category = D_v_sttms_grisela_detail(l_index).category
      ...
      ELSIF p_Action_Code = Cspks_Req_Global.p_delete THEN
         ...
         DELETE STTM_GRISELA_USHTRIMI2 WHERE category = p_Wrk_stdgris2.v_sttms_grisela_ushtrimi2.category
         ;


      ELSIF p_Action_Code IN (Cspks_Req_Global.p_Auth,Cspks_Req_Global.p_Close,Cspks_Req_Global.p_Reopen ) THEN
         l_key := Cspks_Req_Utils.Fn_Get_Item_Desc(p_source,g_ui_name,'STTMS_GRISELA_USHTRIMI2.CATEGORY')||'-'||
         p_Wrk_stdgris2.v_sttms_grisela_ushtrimi2.category;
         BEGIN
            SELECT ROWID
            INTO   l_row_id
            FROM  STTM_GRISELA_USHTRIMI2
      ...
      IF  p_Action_Code IN (Cspks_Req_Global.p_New,Cspks_Req_Global.p_Modify,Cspks_Req_Global.p_Close,Cspks_Req_Global.p_Reopen,Cspks_Req_Global.p_Auth,Cspks_Req_Global.p_Delete,Cspks_Req_Global.p_Version_Delete) THEN
         ...
         IF l_Key_Id IS NOT NULL THEN
            ...
            IF Cspks_Req_Utils.Fn_Field_Log_Reqd(p_Function_Id,p_Action_Code) THEN
               ...
               IF p_Prev_stdgris2.v_sttms_grisela_ushtrimi2.category IS NOT NULL THEN
               ...
               IF l_Prev_Found   THEN
                  l_Dtl_key := '~STTM_GRISELA_USHTRIMI2~'||p_Prev_stdgris2.v_sttms_grisela_ushtrimi2.category||'~';
               ELSIF l_Wrk_Found   THEN
                  l_Dtl_key := '~STTM_GRISELA_USHTRIMI2~'||p_Wrk_stdgris2.v_sttms_grisela_ushtrimi2.category||'~';
               END IF;
...
Pr_Log_Change('ONCE_AUTH',l_Rec_Action,p_Prev_stdgris2.v_sttms_grisela_ushtrimi2.once_auth,p_Wrk_stdgris2.v_sttms_grisela_ushtrimi2.once_auth);
               ...
               IF l_Wrk_count > 0 THEN
                  FOR l_Index IN 1..l_Wrk_count  LOOP
                     l_Dtl_key := '~STTM_GRISELA_DETAIL~'||p_wrk_stdgris2.v_sttms_grisela_detail(l_index).category||'~'||p_wrk_stdgris2.v_sttms_grisela_detail(l_index).product_code||'~';
                     l_Rec_Found := FALSE;
                     l_Rec_Modified := FALSE;
                     IF l_Prev_count > 0 THEN
                        FOR l_Index1 IN 1..l_Prev_count  LOOP
               ...
               IF l_Prev_Count > 0 THEN
                  FOR l_Index IN 1..l_Prev_count  LOOP
                     ...
                     IF NOT l_Rec_Found THEN
                        Dbg('Record Deleted..');
                        l_Dtl_key := '~STTM_GRISELA_DETAIL~'||p_Prev_stdgris2.v_sttms_grisela_detail(l_Index).category||'~'||p_Prev_stdgris2.v_sttms_grisela_detail(l_Index).product_code||'~';
   ...
   BEGIN
      ...
      p_Node_Data(l_Cntr).Query_Node := '0';
      p_Node_Data(l_Cntr).Node_Fields := 'CATEGORY~PRODUCT_CODE~';
      p_Node_Data(l_Cntr).Node_Tags := 'CATEGORY~PRODUCT_CODE~';
